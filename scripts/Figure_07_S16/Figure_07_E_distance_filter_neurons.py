...
### define function-------------------------------------------------------------
def distance_filter_neuron(coords, root_ids, r_um=5000):
    neurons = root_ids
...
close_pt, far_pt = distance_filter_neuron(coords, neurons, r_um=14000)#change if distance is different
#v783 skeleton coordinates are in nm while v680 are in µm
if len(close_pt)>0:
...
df_close.to_csv("./input/tmp/tmp_close_pt_ids_v"+v+".csv")

df_far = df.query("PointNo in @far_pt_ids")

df_far.to_csv("./input/tmp/tmp_far_pt_ids_v"+v+".csv")
